...
def generate_report(java_project_path, output_doc_path, requirements_question, image_class_diagram_path, image_result_path):
    # 完善信息
    requirements_question += ",给出这个实验的实验目的，三条左右，不要带有任何格式"
    summary_question = requirements_question + "，给出这个实验的实验总结，150字左右，只给出一段话即可不带有任何格式，不要加标题，这个回答中不要带有实验目的了，回答以我们为第一人称"

    # 定义api_key
    api_key = "sk-JGR1yG7xV86kqxDcty03hwEAWhXhaVnwcxEKGU0Fp40rAbgs"
    base_url = "https://api.moonshot.cn/v1"  # 确保URL是正确的
    ...
    requirements_completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": requirements_question},
        ],
        temperature=0.3,
    )
    ...
    doc.add_picture(image_class_diagram_path, width=Inches(6))

    # 整合Java项目代码
    java_files_content = ""
    for root, dirs, files in os.walk(java_project_path):
        for file in files:
            if file.endswith('.java') or file.endswith('.h') or file.endswith('.cpp') or file.endswith('.c'):
    ...
    doc.add_picture(image_result_path, width=Inches(6))
...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        self.label2 = QLabel('Java/C++项目路径:', self)
